...
> `near`用于查询某一时间戳附近最近的n条数据，语法与sql的 `in` 查询类似。


-  `near` 与 `in` 的区别

  - `in` 表示完全命中查询、`near` 表示附近数据命中查询
  - `in` 最多返回 `n` 条命中、`near` 最多返回 `2n+2` 条命中
...
- 示例数据：
  ...
  ```shell
  curl --location --request GET 'http://127.0.0.1:8567/v1/query?sql=select * from `t01.p01.a01.d01.p01` where `ts` near (16627392922001,16627392921001)'
  ```
